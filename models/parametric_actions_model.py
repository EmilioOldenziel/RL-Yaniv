...
class TorchParametricActionsModel(DQNTorchModel):
    ...
    def __init__(
        self, obs_space, action_space, num_outputs, model_config, name, true_obs_shape=(4,), action_embed_size=2, **kw
    ):
...
class TorchParametricActionsEmbeddingsModel(DQNTorchModel):
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        # Compute the predicted action embedding
        action_embed, _ = self.action_embed_model({"obs": input_dict["obs"]["observation"]})
